...
class LivestockCustomIdController {
  ...
  async getCustomIdByFarmAndType(req, res) {
    try {
      ...
      if (!customId) {
        return res.status(404).json({ success: false, message: 'Custom ID not found' });
      }
  ...
  async createCustomId(req, res) {
    try {
      ...
      res.status(201).json({success: true, data: customId});
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
   ...
   */
    async changePrefix(req, res) {
        ...
        return res.status(200).json({ success: true, message: 'Prefix updated successfully and last_number reset to 0' });
        } catch (error) {
